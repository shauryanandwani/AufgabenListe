 ...
 const onSubmitForm = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/todos`,
        {
          //description
           description: description.trim(), completed: false,
        });
                  ...
                  {editingTodo === todo.todo_id ? (
                    <div className = "flex items-center gap-x-3">
                      ...
                      onChange = {(e) => setEditedText(e.target.value)}/>
                       <div>
<button onClick={() => saveEdit(todo.todo_id)} className = "px-4 py-2 bg-green-500 text-white rounded-lg mr-2 mt-2 hover:bg-green-400 duration-200"> 
  <MdOutlineDone/> 
</button>
<button onClick={() => setEditingTodo(null)}
className = "px-4 py-2 bg-red-500 text-white rounded-lg mr-2 mt-2 hover:bg-red-400 duration-200">
<IoClose />
</button>
                       </div>
                    ...
                    <div className = "flex justify-between items-center">
                      <div className = "text-2xl flex items-center gap-x-3 overflow-hidden font italic text-gray-700" >
                  <button 
                  onClick = {() => toggleCompleted(todo.todo_id)}
                  className= {`flex-shrink-0 h-5 w-5 border-2 rounded-full flex items-center justify-center ${todo.completed ? "bg-green-500 border-green-500 text-white" : "border-gray-400 hover:border-green-500"}`}>
                ...
                <div className ="flex gap-x-1"> 
                    ...
                    <button onClick={() => {
                      ...
                      className = "p-1 h-5 w-5 text-gray-900 hover:text-gray-900 rounded-lg hover:bg-gray-200 duration-200" > 
                      <MdModeEditOutline /> 
                      </button>

                    <button onClick = {() => deleteTodo(todo.todo_id)} className = "p-1 h-5 w-5 text-red-500 hover:text-red-700 rounded-lg hover:bg-red-50 duration-200"> 
                    <FaTrash /> </button>
                  </div>
                    </div>
